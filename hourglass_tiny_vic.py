...
class HourglassModel():
    ...
    """
    def __init__(self, nFeat = 512, nStack = 4, nModules = 1, nLow = 4, outputDim = 16, batch_size = 16, drop_rate = 0.2, lear_rate = 2.5e-4, decay = 0.96, decay_step = 2000, dataset = None, training = True, w_summary = True, logdir_train = None, logdir_test = None,tiny = True, attention = False,modif = True,w_loss = False, name = 'tiny_hourglass',  joints = ['fru','frd','flu','fld','bru','brd','blu','bld']):
        """ Initializer
        Args:
    ...
    def generate_model(self):
        ...
        with tf.device(self.gpu):
            ...
            if self.attention:
                self.output, self.domain = self._graph_mcam(self.img)
            else :
            ...
            with tf.name_scope('loss'):
                if self.w_loss:
                    ...
                    #first I will try without the sigma value
                    gamma=0.1135
                    ...
                    # the previous commented code is equivalent to this one when gamma==1
                    #hm_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.output, labels=self.gtMaps)
                    #domain_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.domain, labels=self.gtDomain)
                    #self.loss = tf.reduce_mean (hm_loss, name='cross_entropy_loss') + tf.reduce_mean(domain_loss, name='crosse_entropy_domain_loss')
        ...
        with tf.name_scope('Train'):
            ...
            for epoch in range(nEpochs):
                ...
                for i in range(epochSize):
                    ...
                    sys.stdout.write('\r Train:' + '>' '{0}'.format("#"*num) + "{0}<".format(" "*(20-num)) + '||' + str(percent)[:4] + '%' + ' -Epoch:' + str(epoch) + '/' + str(nEpochs) + ' -cost: ' + str(cost)[:6] + ' -avg_loss: ' + str(avg_cost)[:5] + ' -timeToEnd: ' + str(tToEpoch) + ' sec.')
                    sys.stdout.flush()

                    img_train, gt_train, weight_train, gt_domain = next(self.generator)
                    if i % saveStep == 0:
                        if self.w_loss:
                            _, c, summary = self.Session.run([self.train_rmsprop, self.loss, self.train_op], feed_dict = {self.img : img_train, self.gtMaps: gt_train, self.weights: weight_train, self.gtDomain: gt_domain})
                        else:
                            _, c, summary = self.Session.run([self.train_rmsprop, self.loss, self.train_op], feed_dict = {self.img : img_train, self.gtMaps: gt_train, self.gtDomain: gt_domain})
                        ...
                        if self.w_loss:
                            _, c, = self.Session.run([self.train_rmsprop, self.loss], feed_dict = {self.img : img_train, self.gtMaps: gt_train, self.weights: weight_train, self.gtDomain: gt_domain})
                        else:    
                            _, c, = self.Session.run([self.train_rmsprop, self.loss], feed_dict = {self.img : img_train, self.gtMaps: gt_train, self.gtDomain: gt_domain})
                    cost += c
                ...
                if self.w_loss:
                    weight_summary = self.Session.run(self.weight_op, {self.img : img_train, self.gtMaps: gt_train, self.weights: weight_train, self.gtDomain: gt_domain})
                else :
                    weight_summary = self.Session.run(self.weight_op, {self.img : img_train, self.gtMaps: gt_train, self.gtDomain: gt_domain})
                ...
                for i in range(validIter):
                    ...
                    accuracy_pred = self.Session.run(self.joint_accur, feed_dict = {self.img : img_valid, self.gtMaps: gt_valid, self.gtDomain: gt_domain_valid})
                ...
                valid_summary = self.Session.run(self.test_op, feed_dict={self.img : img_valid, self.gtMaps: gt_valid, self.gtDomain: gt_domain_valid})
    ...
    def _graph_hourglass(self, inputs):
        ...
        with tf.name_scope('model'):
            ...
            if self.tiny:
                ...
                with tf.name_scope('stacks'):
                    with tf.name_scope('stage_0'):
                        # inisialisation of the first layer
                        ...
                        out[0] = self._conv_bn_relu(ll[0], self.outDim, 1, 1, 'VALID', 'out')
                        domain[0] = self._conv_bn_relu(flip_gradient(ll[0]), 1,  ll[0].get_shape().as_list()[1], 1, 'VALID', 'out')
                        out_[0] = self._conv(out[0], self.nFeat, 1, 1, 'VALID', 'out_')
                        sum_[0] = tf.add_n([out_[0], r3, ll_[0]], name='merge')
                        
                    for i in range(1, self.nStack -1):
                        # creation of the others layers
                        ...
                        with tf.name_scope('stage_' + str(i)):
                            ...
                            out[i] = self._conv_bn_relu(ll[i], self.outDim, 1, 1, 'VALID', 'out')
                            domain[i] = self._conv_bn_relu(flip_gradient(ll[i]), 1,  ll[i].get_shape().as_list()[1], 1, 'VALID', 'out')
                            out_[i] = self._conv(out[i], self.nFeat, 1, 1, 'VALID', 'out_')
                    ...
                    with tf.name_scope('stage_' + str(self.nStack -1)):
                        ...
                        ll[self.nStack - 1] = self._conv_bn_relu(drop[self.nStack-1], self.nFeat, 1, 1, 'VALID', 'conv')
                        out[self.nStack - 1] = self._conv_bn_relu(ll[self.nStack - 1], self.outDim, 1,1, 'VALID', 'out')
                        domain[self.nStack - 1] = self._conv_bn_relu(flip_gradient(ll[self.nStack - 1]), 1, ll[self.nStack-1].get_shape().as_list()[1],1, 'VALID', 'out')
                return tf.nn.sigmoid(tf.stack(out, axis= 1 , name= 'stack_output'),name = 'final_output'), tf.nn.sigmoid(tf.contrib.layers.fully_connected(tf.layers.flatten(tf.stack(domain, axis= 1 , name= 'stack_domain')),1),name = 'final_domain')
    ...
    def _conv_block(self, inputs, numOut, name = 'conv_block'):
        ...
        if self.tiny:
            ...
            with tf.name_scope(name):
                # this code will do approximally the same think than the code above but with more layers
                with tf.name_scope('norm_1'):
                    # bash and convolution layers
                ...
                with tf.name_scope('norm_2'):
                    # bash, padding and convolution layers
