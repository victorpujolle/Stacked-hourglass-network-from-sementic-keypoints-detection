                ...
                with tf.name_scope('stacks'):
                    with tf.name_scope('stage_0'):
                        ...
                        if self.modif:
                            ...
                            out[0] = self._conv(ll[0], self.outDim, 1, 1, 'VALID', 'out')
                            domain[0] = self._conv(flip_gradient(ll[0]), 1, ll[0].get_shape().as_list()[1], 1, 'VALID', 'out')
                        out_[0] = self._conv(out[0], self.nFeat, 1, 1, 'VALID', 'out_')
                    ...
                    for i in range(1, self.nStack -1):
                        with tf.name_scope('stage_' + str(i)):
                            ...
                            if self.modif:
                                ...
                                out[i] = self._conv(ll[i], self.outDim, 1, 1, 'VALID', 'out')
                                domain[i] = self._conv(flip_gradient(ll[i]), 1,  ll[i].get_shape().as_list()[1], 1, 'VALID', 'out')
                            out_[i] = self._conv(out[i], self.nFeat, 1, 1, 'VALID', 'out_')
                    ...
                    with tf.name_scope('stage_' + str(self.nStack -1)):
                        ...
                        if self.modif:
                            ...
                            out[self.nStack - 1] = self._conv(ll[self.nStack - 1], self.outDim, 1,1, 'VALID', 'out')
                            domain[self.nStack - 1] = self._conv(flip_gradient(ll[self.nStack - 1]), 1, ll[self.nStack-1].get_shape().as_list()[1],1, 'VALID', 'out')
                if self.modif:
                    ...
                    return tf.stack(out, axis= 1 , name = 'final_output'), tf.contrib.layers.fully_connected(tf.layers.flatten(tf.stack(domain, axis= 1 )),1, scope = 'final_domain')
